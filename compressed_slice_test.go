...
func TestCompressedIntBufferBasic(t *testing.T) {
	var cs compslices.CompressedSlice[int]
...
func testCompressedBuffer[T compslices.PackType](bufs [][]T, withMinMax bool, t *testing.T) {
	// create a new compressed buffer with minmax enabled
	var cb compslices.CompressedSlice[T]
	cb.WithMinMax(true)
	...
	// check AppendOne
	var cb2 compslices.CompressedSlice[T]
...
func TestLShift(t *testing.T) {
	buf := genBuf[int](0, 10)
	var cb compslices.CompressedSlice[int]
	cb = cb.Append(buf)
	...
	cb = compslices.CompressedSlice[int]{}
...
func TestCompressBufferConstant(t *testing.T) {
	...
	var cb compslices.CompressedSlice[int64]
...
func testCompressBufferIntFull[T compslices.PackType](t *testing.T) {
...
func TestImportExportSlice(t *testing.T) {
	var cs, cs2 compslices.CompressedSlice[int]
	var testInput = []int{16985, 17312, 17639, 17966, 18293, 18620, 18947, 19274, 19601, 19928, 20255, 20582, 20785, 21012, 21339, 21666, 21993, 22320, 22647, 22974, 23301, 23628, 23955, 24282, 24609, 24936, 25263, 25590, 25917, 26244, 26571, 26898, 27225, 27552, 27879, 28206, 28533, 28860, 29187, 29514, 29841, 30168, 30495, 30822, 31149, 31476, 31803, 32130, 32457, 32784, 33111, 33438, 33765, 34092, 34419, 34746, 35073, 35400, 35727, 36054, 36381, 36708, 37035, 37362}
	cs = cs.Append(testInput)
...
func checkEqual[T compslices.PackType](got, want []T, precision float64) bool {
	if len(got) != len(want) {
		return false
	}
	for i := range got {
...
func genBuf[T compslices.PackType](start, size int) []T {
	buf := make([]T, size)
	for i := 0; i < size; i++ {
		buf[i] = T(start + i)
	}
...
var (
	sampleTimeSerieData []float64 = []float64{0.0491, 0.0491, 0.0491, 0.0491, 0.0491, 0.0491, 0.0458, 0.0421, 0.0367, 0.0349, 0.0334, 0.0334, 0.0334, 0.0334,
		...
		0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0183, 0.0183, 0.0183, 0.0183,
		0.0183, 0.0183, 0.0183, 0.0183, 0.0183, 0.0183, 0.0183, 0.0183, 0.0183, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184,
		0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0184, 0.0183, 0.0183, 0.0183, 0.0183, 0.0183, 0.0147, 0.0107, 0.00362, 0.000764, 0.0000762,
		...
		0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0166, 0.0134, 0.0105, 0.00581,
		0.00387, 0.000225, 0.000225, 0.000225, 0.000234, 0.000234, 0.000234, 0.000234, 0.000234, 0.000234, 0.000234, 0.000234, 0.000234, 0.000234,
